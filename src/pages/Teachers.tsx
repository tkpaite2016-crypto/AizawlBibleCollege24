            ...
            {loading ? (
              ...
              <div className="space-y-8">
                {filtered.map((member, index) => {
                  ...
                  return (
                    <div key={member.id} className="card hover:shadow-lg transition-all duration-300 relative">
                      <div className={`absolute top-0 ${imageLeft ? 'right-0' : 'left-0'} w-1.5 h-full bg-gold-400 ${imageLeft ? 'rounded-r-xl' : 'rounded-l-xl'} hidden md:block z-10`} />
                      <div className="flex flex-col md:flex-row items-stretch">
                        {/* Photo */}
                        <div className={`w-full md:w-56 lg:w-72 flex-shrink-0 bg-slate-100 relative overflow-hidden min-h-[200px] md:min-h-0 rounded-t-xl md:rounded-none ${imageLeft ? 'md:order-1' : 'md:order-2'}`}>
                          {member.photo_url ? (
                            <img
                              src={member.photo_url}
                        ...
                        {/* Text */}
                        <div className={`flex-1 p-6 md:p-8 flex flex-col justify-center relative ${imageLeft ? 'md:order-2' : 'md:order-1'}`}>
